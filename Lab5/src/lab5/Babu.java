...
public class Babu {
		...
		while (voidCounter < 35) {
			...
			if (getColor() >= 28) {
				...
				while (voidCounter < 35) {
					...
					while ((getDistance() - THRESHOLD > ERROR_THRESHOLD)	&& (getDistance() < 10)) {

						robot.setSpeeds(3, 5);

					}
					// Robot needs to turn left
					while (getDistance() - THRESHOLD < -1 * ERROR_THRESHOLD) {
				...
				while (voidCounter < 55) {
					...
					// Robot needs to turn right
					while ((getDistance() - THRESHOLD > ERROR_THRESHOLD)
							&& (getDistance() < 10)) {
			...
			while ((getDistance() - THRESHOLD > ERROR_THRESHOLD && blockFrontCase)
					&& (getDistance() < 15)) {
			...
			if (getDistance() >= 15 && blockFrontCase) {
	...
	private int getColor(){
		float colorData[] = new float[cs.getAmbientMode().sampleSize()];
